...
#define Rail_GUN_PIN1     67

#define Rail_GUN_PIN2     60
...
int ioDevices_thread_init(void)
{
   rt_thread_t ioDecices_tid;
    ...
    if (ioDecices_tid != RT_NULL){			
				...
				rt_pin_mode(Rail_GUN_PIN1, PIN_MODE_OUTPUT);  //WIFI 连接IO检测
				rt_pin_write(Rail_GUN_PIN1 ,PIN_LOW);
				rt_pin_mode(Rail_GUN_PIN2, PIN_MODE_OUTPUT);  //WIFI 连接IO检测
				rt_pin_write(Rail_GUN_PIN2 ,PIN_LOW);
...
void rail_gun_start_charge(void)
	{
		...
		rt_pin_write(Rail_GUN_PIN2 ,PIN_HIGH);
...
void rail_gun_start_up(void)
	{
		...
		rt_pin_write(Rail_GUN_PIN1 ,PIN_HIGH);
