...
void uart_send_hmi_is_short(uint8 N_number)  //发送给hmi 是否短路
{ 	
		if(1 == ShortFlag)//当短路
		{
		...
		rt_device_write(focus_uart_device, 0,him_uart_short_cmd	, sizeof(him_uart_short_cmd));
...
{
		him_ret_status = 0x03;
		uart_send_hmi_writer_status(&him_ret_status);//正在写入
